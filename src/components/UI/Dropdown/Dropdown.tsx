...
const Dropdown: React.FC<DropdownProps> = ({
    ...
    return (
        <div ref={ref} className={["dropdown", dropdownCss].join(" ")}>
            ...
            {open && (
                <div ref={menuRef} className={menuCss}
                    ...
                    {openSubmenus.map((menu, idx) => {
                        const currentParentItem =
                            activePanes
                                .flatMap((pane) => pane.menuItems)
                                .find((item) => item.id === menu.item.id) ?? menu.item;

                        return (
                            <div
                                key={`${menu.level}-${currentParentItem.id ?? idx}`}
                            ...
                            >
                                {currentParentItem.children?.map((child, idx) => (
                                    <DropdownItem
                                        key={`${child.id}-${child.selected}`}
                                        item={child}
                                        level={menu.level + 1}
                                        onExpand={handleExpand}
                                        closeDropdown={() => closeDropdown()}
